...
const GetAllUsuarios = async (req, res) => {
  try {
    ...
    res.status(500).json({ status: "erro", mensagem: "Erro ao obter todos os usuarios", erro: error.message });
  }
};
...
const GetUsuarioByID = async (req, res) => {
  try {
    const usuarioId = (req.params.id);  // Assegure-se de que está enviando um ID no corpo da requisição
    const registro = await mdlUsuario.GetUsuarioByID(usuarioId);
    if (registro) {
      res.json({ status: "ok", registro });
    } else {
      res.status(404).json({ status: "erro", mensagem: "Usuario com esse ID não encontrada"});
    }
  } catch (error) {
    res.status(500).json({ status: "erro", mensagem: "Erro ao buscar usuario", erro: error.message });
  }
};
...
const InsertUsuario = async (req, res) => {
  try {
    ...
    res.status(500).json({ status: "erro", mensagem: "Erro ao inserir usuario", erro: error.message });
  }
};
...
const UpdateUsuario = async (req, res) => {
    try {
      ...
      if (linhasAfetadas > 0) {
        res.status(200).json({ status: msg, mensagem: "Usuário atualizado com sucesso", linhasAfetadas });
      } else {
        res.status(404).json({ status: "erro", mensagem: "Usuário não encontrado" });
      }
    } catch (error) {
      // Retorna erro em caso de exceção
      res.status(500).json({ status: "erro", mensagem: "Erro ao atualizar usuário", erro: error.message });
    }
  };
